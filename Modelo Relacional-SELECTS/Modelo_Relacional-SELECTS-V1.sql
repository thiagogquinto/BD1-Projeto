...
-- 1 - Selecione todos os cozinheiros, menos os cozinheiros que  possuem como especialidade sobremesa
SELECT * FROM COZINHEIRO C, FUNCIONARIO F WHERE C.cpf = F.cpf AND F.cpf NOT IN (SELECT C2.cpf FROM COZINHEIRO C2 WHERE C2.especialidade = 'Sobremesa');

-- 2 - Selecione os nomes dos clientes, a mesa usada e o valor da conta maior valor registrada
SELECT C.nome AS CLIENTE, CO.valor_total AS VALOR, M.numero AS NÚMERO_MESA FROM CLIENTE C, MESA M, CONTA CO WHERE
 M.numero = CO.numero_mesa AND C.numero_conta = CO.numero AND CO.valor_total = (SELECT MAX(C2.valor_total) FROM CONTA C2);

-- 3 - Selecione os garçons que são avaliados como 'Ruim' e já atenderam ao menos 3 clientes
SELECT DISTINCT F.nome FROM GARCOM G, FUNCIONARIO F, GARCOM_CLIENTE GC WHERE F.cpf = G.cpf AND G.avaliacao = 'Ruim'
 AND G.cpf = GC.cpf_G AND GC.cpf_G IN
(SELECT GC2.cpf_G FROM GARCOM_CLIENTE GC2 GROUP BY GC2.cpf_G HAVING COUNT(GC2.cpf_G) > 2);

-- 4 - Selecione os clientes que possuem contas registradas com o valor maior que a média de valor das contas
SELECT nome, valor_total FROM CLIENTE C, CONTA CT WHERE C.numero_conta = CT.numero AND CT.valor_total > 
(SELECT AVG(CT2.valor_total) FROM CONTA CT2);

-- 5 - Selecione o cozinheiro que fizeram produtos que usam carne bovina como ingrediente, selecione também o nome desses 
-- produtos
SELECT DISTINCT nome AS Cozinheiro, CP.nome_P AS Produto FROM COZINHEIRO_PRODUTO CP, PRODUTO_INGREDIENTE PRI, 
COZINHEIRO C, FUNCIONARIO F WHERE CP.cpf_C = C.cpf AND C.cpf = F.cpf AND CP.nome_P IN
 (SELECT CP2.nome_P FROM COZINHEIRO_PRODUTO CP2, PRODUTO_INGREDIENTE P2 WHERE CP2.nome_P = P2.nome_P
 AND P2.nome_I = 'Carne Bovina'); 

-- 6 - Selecione os ingrediente, produto e o tempo de preparo do produto que leva mais tempo para ser preparado
SELECT PIN.nome_I AS Ingrediente, P.nome AS Produto, P.tempo_preparo FROM PRODUTO P, PRODUTO_INGREDIENTE PIN WHERE PIN.nome_P = P.nome AND P.tempo_preparo
 = (SELECT MAX(P2.tempo_preparo) FROM PRODUTO P2);
 
 -- 7 - Selecione os cozinheiros que não prepararam nenhum prato
 SELECT DISTINCT F.nome FROM COZINHEIRO C, FUNCIONARIO F WHERE C.cpf = F.cpf AND C.cpf NOT IN
 (SELECT CP2.cpf_C FROM COZINHEIRO_PRODUTO CP2);
 
 -- 8 - Selecione os fornecedores e sua cidade que fornecem mais de um produto, além do fornecedor que
 --      fornece o ingrediente mais caro
 SELECT DISTINCT F.nome AS Nome_Fornecedor, E.cidade AS Cidade_Fornecedor FROM FORNECEDOR F, ENDERECO E, 
 FORNECEDOR_INGREDIENTE FI WHERE E.cnpj_F = F.cnpj 
 AND F.cnpj = FI.cnpj_F AND FI.cnpj_F IN
 (SELECT FI2.cnpj_F FROM FORNECEDOR_INGREDIENTE FI2 GROUP BY FI2.cnpj_f HAVING COUNT(FI2.cnpj_F) > 1)
 UNION
 SELECT F2.nome, E2.cidade FROM FORNECEDOR F2, ENDERECO E2, FORNECEDOR_INGREDIENTE FI2, INGREDIENTE I2
 WHERE E2.cnpj_F = F2.cnpj AND F2.cnpj = FI2.cnpj_F AND FI2.nome_I = I2.nome AND I2.valor_unitario IN
 (SELECT MAX(I3.valor_unitario) FROM INGREDIENTE I3);
 
 -- 9 - Selecione a descricao do pedido, o caixa e seu saldo final relacionada à conta de maior valor com status de paga
 SELECT P.descricao, C.numero AS numero_caixa, C.saldo_final FROM PEDIDO P, CONTA CT, CAIXA C 
 WHERE P.numero_conta = CT.numero AND CT.statu = 'Paga' AND CT.numero_caixa = C.numero AND CT.valor_total = 
 (SELECT MAX(CT2.valor_total) FROM CONTA CT2);
 
 -- 10 - Selecione os clientes que form atendidos pelos funcionários com avaliação boa ou regular
 SELECT C.nome, C.cpf, C.telefone FROM CLIENTE C, GARCOM G, GARCOM_CLIENTE GC 
 WHERE GC.cpf_G = G.cpf AND GC.cpf_C = C.cpf AND G.avaliacao = 'Bom'
 UNION
SELECT C2.nome, C2.cpf, C2.telefone FROM CLIENTE C2, GARCOM G2, GARCOM_CLIENTE GC2 
WHERE GC2.cpf_G = G2.cpf AND GC2.cpf_C = C2.cpf AND G2.avaliacao = 'Regular';